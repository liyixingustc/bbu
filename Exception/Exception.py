...
class ExceptionCustom:
    ...
    exceptions = [
        {'code': '1001', 'class': OperationalError, 'error': 'database is locked', 'client_message': 'Database is busy now, please wait for some seconds, refresh the page and try again.'}
    ]

    @classmethod
    def get_client_message(cls, e):
        for exception_ in cls.exceptions:
            if exception_['error'] == str(e):
